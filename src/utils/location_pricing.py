"""
Location-based pricing utilities for regional cost adjustments.
"""
import json
import os
from typing import Optional, Tuple

# Simple zipcode to state mapping (first 3 digits)
# This is a simplified version - in production you'd use a complete database
ZIPCODE_TO_STATE = {
    ...
    "320": "FL", "321": "FL", "322": "FL", "323": "FL", "324": "FL", "325": "FL", "326": "FL", "327": "FL", "328": "FL", "329": "FL", "330": "FL", "331": "FL", "332": "FL", "333": "FL", "334": "FL", "335": "FL", "336": "FL", "337": "FL", "338": "FL", "339": "FL", "340": "FL", "341": "FL", "342": "FL", "343": "FL", "344": "FL", "345": "FL", "346": "FL", "347": "FL", "349": "FL",
...
def zipcode_to_state(zipcode: str) -> Optional[str]:
    """Convert a 5-digit zipcode to a 2-letter state code."""
    if not zipcode or len(zipcode) != 5:
        return None
    
    # Try first 3 digits
    prefix = zipcode[:3]
    return ZIPCODE_TO_STATE.get(prefix)


def load_state_multipliers() -> dict:
    """Load state cost multipliers from JSON file."""
    current_dir = os.path.dirname(os.path.abspath(__file__))
    data_path = os.path.join(current_dir, '..', 'data', 'state_cost_multipliers.json')
    
    with open(data_path, 'r') as f:
        return json.load(f)


def get_cost_multiplier(zipcode: Optional[str] = None, state_code: Optional[str] = None) -> Tuple[float, str]:
    """
    Get cost multiplier for a location.
    
    Args:
        zipcode: Optional 5-digit zipcode
        state_code: Optional 2-letter state code
        
    Returns:
        Tuple of (multiplier, location_name)
        Returns (1.0, "National Average") if location cannot be determined
    """
    multipliers = load_state_multipliers()
    
    # Try to get state from zipcode first
    if zipcode:
        state = zipcode_to_state(zipcode)
        if state and state in multipliers['states']:
            state_data = multipliers['states'][state]
            return (state_data['multiplier'], state_data['name'])
    
    # Try direct state code
    if state_code and state_code in multipliers['states']:
        state_data = multipliers['states'][state_code]
        return (state_data['multiplier'], state_data['name'])
...
def apply_location_pricing(base_cost: float, zipcode: Optional[str] = None, state_code: Optional[str] = None) -> Tuple[float, float, str]:
    """
    Apply location-based pricing adjustment to a base cost.
    
    Args:
        base_cost: Base cost before location adjustment
        zipcode: Optional 5-digit zipcode
        state_code: Optional 2-letter state code
        
    Returns:
        Tuple of (adjusted_cost, multiplier, location_name)
    """
    multiplier, location = get_cost_multiplier(zipcode, state_code)
    adjusted_cost = base_cost * multiplier
    return (adjusted_cost, multiplier, location)
